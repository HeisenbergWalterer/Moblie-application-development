...
@MainActor final class ChatViewModel: ObservableObject {
    ...
    func send(context: NSManagedObjectContext) async {
        ...
        // 验证配置
        if cfg.host.isEmpty {
            print("❌ 错误：API Host 未配置")
            let reply = AIMessage(role: .assistant, content: "❌ 错误：未配置 API Host", date: Date())
            messages.append(reply)
            sending = false
            return
        }
        if cfg.textModel.isEmpty {
            print("❌ 错误：文本模型未配置")
            let reply = AIMessage(role: .assistant, content: "❌ 错误：未配置文本模型", date: Date())
            messages.append(reply)
            sending = false
            return
        }
        ...
        if apiKey.isEmpty {
            ...
            let reply = AIMessage(role: .assistant, content: "❌ 错误：未配置 API Key，请在设置中保存", date: Date())
            messages.append(reply)
            sending = false
            return
        }
        ...
        print("📤 发送消息: \(trimmed)")
        print("🔧 配置 - Host: \(cfg.host), 模型: \(cfg.textModel)")
        
        do {
            let resp = try await client.sendChat(messages: messages, summary: summary, config: cfg)
            print("✅ 收到响应: \(resp.text.prefix(100))")
            ...
            for ch in resp.text {
                if let lastIndex = messages.indices.last { 
                    messages[lastIndex].content.append(ch)
                    streamingTick &+= 1
                }
            ...
            let reply = AIMessage(role: .assistant, content: "❌ 错误: \(error.localizedDescription)", date: Date())
            messages.append(reply)
        }
